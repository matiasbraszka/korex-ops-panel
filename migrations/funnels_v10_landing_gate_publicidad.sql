-- Funnels v10 (aplicada a prod vía MCP el 2026-07-29): la Landing NO se marca
-- "Diseñada" si el funnel no tiene el enlace de Publicidad (strategy_pages.ads_url,
-- la URL que se pega en el anuncio de Meta). Regla pedida por Matías: página al
-- aire sin ads_url = pendiente, sub-estado 'pagina' ("falta publicidad").
...
AS $function$
  with f as (
    select
      ...
      (coalesce(p.ads_url,'') <> '') as has_ads_url
  ...
  m as (
    select f.*,
      ...
      case when f.landing_pub and f.has_ads_url then 'disenado'
           when f.landing_pub then 'pagina'
           else 'nada' end as landing_sub
    from f
  ),
  g as (
    ...
    from m cross join lateral (values
      ...
      ('landing', 6, 'Landing / BCL',
        (m.landing_pub and m.has_ads_url), m.vsl_guionado, m.landing_sub,
        case m.landing_sub when 'disenado' then 'Diseñada (página al aire + enlace de Publicidad)'
             when 'pagina' then 'Página al aire pero FALTA el enlace de Publicidad (URL del anuncio de Meta)'

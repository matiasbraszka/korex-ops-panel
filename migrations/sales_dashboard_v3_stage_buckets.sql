-- sales_dashboard_v3_stage_buckets
--
-- Agrega columna `bucket` a sales_pipeline_stages para que cada etapa de cada
-- pipeline indique explicitamente a que fase universal pertenece. Asi el
-- dashboard "Todos los CRMs" agrupa correctamente sin importar la posicion
-- ni el nombre de la etapa.
--
-- Buckets validos: 'inicial' | 'en_proceso' | 'por_cerrar' | 'cerrados'.
--
-- Backfill: las etapas existentes se autoclasifican por la posicion relativa
-- (misma heuristica que estaba en la RPC v2). Despues el usuario puede
-- ajustarlas a mano desde el editor de columnas.
...
alter table public.sales_pipeline_stages
  drop constraint if exists sales_pipeline_stages_bucket_check;
alter table public.sales_pipeline_stages
  add constraint sales_pipeline_stages_bucket_check
    check (bucket in ('inicial', 'en_proceso', 'por_cerrar', 'cerrados'));

-- 2) Backfill por posicion relativa
with maxpos as (
  select pipeline_id, max(position) as max_pos
  from public.sales_pipeline_stages
  group by pipeline_id
)
update public.sales_pipeline_stages s
set bucket = case
  when m.max_pos = 0 then 'cerrados'
  when s.position = m.max_pos then 'cerrados'
  when (s.position::float / m.max_pos) < 0.34 then 'inicial'
  when (s.position::float / m.max_pos) < 0.67 then 'en_proceso'
  else 'por_cerrar'
end
from maxpos m
where s.pipeline_id = m.pipeline_id
  and s.bucket is null;

-- Default para futuras inserts sin bucket explicito
alter table public.sales_pipeline_stages
  alter column bucket set default 'en_proceso';

-- 3) Reescribimos la RPC v3: el funnel de "todos los CRMs" agrupa por
--    s.bucket en vez de por posicion calculada. El funnel filtrado a un
--    pipeline sigue mostrando las etapas reales.
...
begin
  ...
  funnel_specific as (
    -- Pipeline filtrado: etapas reales con sus colores
  ...
  funnel_buckets as (
    -- Todos los CRMs: agrupar por s.bucket
    select bucket,
      sum(cnt)::int as cnt,
      sum(amount)::numeric as amount
    from funnel_per_stage

-- ─────────────────────────────────────────────────────────────────────────────
-- portal_v47_siguiente_solo_si_falta.sql
--
-- El "Siguiente: VSL · Reclutamiento" del pie del documento aparecía con que
-- EXISTIERA un guion de VSL marcado para grabar, entregado o no. El cliente
-- terminaba de leer sus anuncios y lo mandábamos a grabar un VSL que ya había
-- grabado y subido.
--
-- Ahora apunta a lo que de verdad le falta, y si no falta nada no aparece (el
-- portal ya oculta el enlace cuando viene null).
--
-- "Falta" es una sola definición, en `_portal_doc_pendiente`:
--   · quedan guiones por grabar — contando, N guiones piden N archivos; o
--   · quedan pestañas por revisar (terminado + revisar, sin marcar).
-- ─────────────────────────────────────────────────────────────────────────────

begin;

create or replace function public._portal_doc_pendiente(p_client text, p_strategy text, p_kind text)
returns boolean
language sql stable security definer
set search_path to 'public', 'pg_temp'
as $$
  select
    (select count(*) from public.del_sections d
      where d.strategy_id = p_strategy and d.kind = p_kind and d.para_grabar)
    > (select count(*) from public.funnel_resources fr
        where fr.strategy_id = p_strategy
          and fr.bucket_key in (case p_kind when 'vsl' then 'vsl_rec'  else 'ad_rec'  end,
                                case p_kind when 'vsl' then 'vsl_edit' else 'ad_edit' end))
  or exists (
    select 1
      from public.del_sections d
      left join public.portal_guion_status gs
        on gs.section_id = d.id and gs.client_id = p_client
     where d.strategy_id = p_strategy and d.kind = p_kind
       and coalesce(d.estado_seccion, '') = 'terminado'
       and coalesce(d.accion_cliente, '') = 'revisar'
       and not coalesce(d.para_grabar, false)
...
begin
  ...
  if v_solo_grabar then
    ...
    -- "Siguiente" solo si al cliente le falta algo ahí. Antes alcanzaba con que
    -- el documento existiera, y lo mandábamos a grabar lo que ya había grabado.
    v_next := null;
    if v_tipo = 'ads' and public._portal_doc_pendiente(v_cid, p_strategy, 'vsl') then
      v_next := jsonb_build_object('strategyId', p_strategy, 'tipo', 'vsl', 'label', 'VSL · ' || v_name);
    elsif v_tipo = 'vsl' and public._portal_doc_pendiente(v_cid, p_strategy, 'anuncios') then
      v_next := jsonb_build_object('strategyId', p_strategy, 'tipo', 'ads', 'label', 'Anuncios · ' || v_name);
    else
      select jsonb_build_object('strategyId', s.id,
               'tipo', case when public._portal_doc_pendiente(v_cid, s.id, 'anuncios') then 'ads' else 'vsl' end,
               'label', s.name)
        into v_next
      from public.strategies s
      where s.client_id = v_cid and s.id <> p_strategy
        and (public._portal_doc_pendiente(v_cid, s.id, 'anuncios')
             or public._portal_doc_pendiente(v_cid, s.id, 'vsl'))

-- Panorama v4 (aplicada a prod vía MCP el 2026-07-29): el cliente viaja con
-- tiene_crm_prod = tiene cargado el acceso "CRM KOREX | PRODUCCIÓN" (clients.links,
-- category 'acceso', label con "crm"+"produc" y contraseña no vacía). Es un dato a
-- nivel CLIENTE (aplica a todos sus funnels). Cambia la firma → drop + create.
drop function if exists clients_panorama();
create function clients_panorama()
returns table(client_id text, client_name text, company text, niche text, n_estrategias integer,
              tiene_logo boolean, tiene_colores boolean, imagenes_files integer, testimonios_files integer,
              tiene_cuenta boolean, tiene_crm_prod boolean, estrategias jsonb)
language sql stable security definer set search_path to 'public' as $function$
  with recursive cl as (
    select id, name, company, niche, jsonb_array_length(coalesce(meta_ads,'[]'::jsonb)) > 0 as tiene_cuenta,
      exists (
        select 1 from jsonb_array_elements(coalesce(links,'[]'::jsonb)) l
        where l->>'category' = 'acceso' and l->>'label' ~* 'crm' and l->>'label' ~* 'produc'
          and coalesce(l->>'password','') <> ''
      ) as tiene_crm_prod
    from clients where drive_folder_url is not null
  ),
  recsub as (
  ...
  order by cl.name;
$function$;
grant execute on function clients_panorama() to authenticated;

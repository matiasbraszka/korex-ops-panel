-- cerebro_pipeline_del_anuncios.sql — que el paso "Anuncios" del pipeline también se marque
-- guionado cuando el copy vive en el DEL (sección kind='anuncios'), no solo en avatars[].ad_script.
-- Es el mismo criterio que ya usa el VSL (sección kind='vsl'). Cambio ADITIVO: sigue contando
-- ad_script como antes, y ADEMÁS reconoce el DEL. Necesario porque ahora los agentes guardan al DEL.
...
AS $function$
  with f as (
    select
      ...
      ((select count(*) from jsonb_array_elements(coalesce(p.avatars,'[]'::jsonb)) a where coalesce(a->>'ad_script','') <> '')>0
        or exists(select 1 from del_sections ds join client_brain_docs cbd on cbd.id=ds.doc_id
                  where cbd.strategy_id=p.strategy_id and cbd.doc_kind='del' and ds.kind='anuncios'
                    and length(regexp_replace(coalesce(ds.html,''),'<[^>]+>','','g')) > 200)) as ads_guionado,

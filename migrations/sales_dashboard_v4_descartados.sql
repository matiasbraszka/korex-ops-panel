-- sales_dashboard_v4_descartados
--
-- Agrega un quinto bucket 'descartados' para etapas tipo "Descartado", "No
-- contesto", "Perdido", etc. Los leads que esten en una etapa con
-- bucket='descartados' se EXCLUYEN del funnel, del donut de calentura, del
-- pipeline total y del ponderado. Siguen contando como "Contactos" del mes
-- porque fueron contactados, pero ya no inflan los numeros de proyeccion.
...
alter table public.sales_pipeline_stages
  add constraint sales_pipeline_stages_bucket_check
    check (bucket in ('inicial', 'en_proceso', 'por_cerrar', 'cerrados', 'descartados'));
...
begin
  ...
  leads_open as (
    -- Abiertos = sin client_id Y sin estar en una etapa descartada
    select * from public.sales_leads
    where pipeline_id = any(visible)
      and (p_pipeline_id is null or pipeline_id = p_pipeline_id)
  ...
  per_owner as (
    select owner_id,
      ...
      -- Pipeline: solo open + no descartado
      coalesce(sum(estimated_value) filter (
        where client_id is null
          and (stage_id is null or stage_id not in (select id from discarded_stages))
      ), 0)::numeric as pipeline,
  ...
  funnel_per_stage as (
    -- Excluimos las etapas con bucket=descartados directamente del funnel.

-- Regla de Matías: las TRANSFERENCIAS nunca son software. Software = cargo
-- automático a la tarjeta (no Facebook). Por eso las transferencias se evalúan
-- ANTES que software, y software sólo atrapa lo que no es transferencia.
...
SET search_path = public, pg_catalog AS $$
  SELECT CASE
    ...
    -- Transferencias salientes NUNCA son software (van antes que la regla de software).
    WHEN p_kind IN ('outgoingPayment','externalTransfer')
      THEN 'Pagos / transferencias externas'
    -- Software: cargo automático a la tarjeta (o 'other'), por proveedor conocido.

...
// El semáforo decide por fallos de las últimas 48 h (no de la semana): un corte puntual
// ya resuelto no debe seguir pintando de amarillo durante 7 días.
function reasonFor(n: Record<string, unknown>): string {
  const health = str(n.health), source = str(n.source), runtime = str(n.runtime);
  const stale = n.data_stale === true;
  const failed = Number(n.failed_48h) || 0;
  const failedWeek = Number(n.failed_7d) || 0;
  if (health === "error") {
    ...
    if (source === "cron" && failed > 0) return `Falla al ejecutarse (${failed} fallos en las últimas 48 h).`;
    return "Está fallando.";
  }
  if (health === "warn") {
    if (stale) return `Corre, pero no está actualizando los datos (último: ${fmtDate(n.last_data as string)}).`;
    if (failed > 0) {
      const extra = failedWeek > failed ? ` (${failedWeek} en los últimos 7 días).` : ".";
      return `Tuvo ${failed} fallos en las últimas 48 h${extra}`;
    }

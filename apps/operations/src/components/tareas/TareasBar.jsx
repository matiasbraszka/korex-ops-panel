...
export default function TareasBar({ view, scope, setScope, onlySprint, setOnlySprint }) {
  const { activeSprint, teamMembers, taskAssignee, setTaskAssignee, closeSprint, hideCompletedTasks, setHideCompletedTasks } = useApp();
  ...
  useEffect(() => {
    ...
    const h = (e) => { if (pRef.current && !pRef.current.contains(e.target)) setPersonOpen(false); };

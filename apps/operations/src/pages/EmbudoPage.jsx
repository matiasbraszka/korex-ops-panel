...
import { ChevronDown, ExternalLink, ArrowDown, AlertTriangle, CheckCircle2, TrendingDown } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data, error } = await supabase
        ...
        .select('id, label, tag_id, client_id, range_30d, last_synced_at, client:clients(name)')
        .eq('active', true);
      if (!active) return;
      ...
      {cur && r && (
        <>
          <div className="grid grid-cols-4 gap-3.5 mb-5">
            ...
            <Kpi label="Llegan al VSL" value={pct(toVsl)} sub={vslStep ? `${fmt(vslStep.count)} de ${fmt(entrada)} entran` : 'sin paso de VSL'} color="#0EA5A5" />

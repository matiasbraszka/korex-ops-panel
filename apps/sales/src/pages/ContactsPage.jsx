...
const CATEGORIES = [
  { id: 'prospect',     label: 'Potencial cliente', color: '#5B7CF5' },
  { id: 'client',       label: 'Cliente',           color: '#22C55E' },
  { id: 'mentor',       label: 'Mentor',            color: '#A855F7' },
  { id: 'partner',      label: 'Socio',             color: '#EAB308' },
  { id: 'team',         label: 'Equipo',            color: '#06B6D4' },
  { id: 'client_team',  label: 'Equipo del cliente', color: '#F97316' },
...
export default function ContactsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return contacts.filter((c) => {
      ...
      if (q) {
        const hay = [
          c.first_name, c.last_name, c.phone, c.email, c.company, c.notes,
  ...
  const handleDelete = async (id) => {
    if (!confirm('¿Eliminar este contacto? Si tiene leads o clientes vinculados se eliminarán los leads.')) return;
      ...
      <div className="shrink-0 space-y-2 mb-2">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div>
            ...
            <p className="text-[11px] text-text3 mt-0.5">{counts.total} contactos en la base</p>
          </div>
      ...
      <div className="flex-1 min-h-0 bg-white border border-border rounded-lg overflow-hidden flex flex-col">
        <div className="overflow-auto flex-1">
          <table className="w-full text-[12px] min-w-[900px]">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={6} className="text-center text-text3 py-8 text-[12px]">Sin resultados</td></tr>
              ) : filtered.map((c) => (
                <tr key={c.id} onClick={() => openEdit(c)}
                  ...
                  <td className="px-3 py-1.5 font-semibold">
                    {[c.first_name, c.last_name].filter(Boolean).join(' ') || <span className="text-text3 italic">Sin nombre</span>}
                  </td>
                  <td className="px-2 py-1.5 text-text2">{c.company || '—'}</td>
                  <td className="px-2 py-1.5 text-text2">{c.phone || '—'}</td>
                  <td className="px-2 py-1.5 text-text2">{c.email || '—'}</td>
                  <td className="px-2 py-1.5">
                    <div className="flex flex-wrap gap-1">
                      ...
                      {(c.categories || []).length === 0 && <span className="text-text3 text-[10px]">—</span>}
                    </div>
                  </td>
                  <td className="px-2 py-1.5 text-right">
                    <button onClick={(e) => { e.stopPropagation(); handleDelete(c.id); }}
                            title="Eliminar"
                            className="opacity-0 group-hover:opacity-100 text-text3 hover:text-red bg-transparent border-0 p-1 cursor-pointer">
                      <Trash2 size={13} />
                    </button>
                  </td>
                </tr>

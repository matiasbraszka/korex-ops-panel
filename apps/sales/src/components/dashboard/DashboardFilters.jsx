...
import { ChevronDown, Users, Check, Settings2, Layers } from 'lucide-react';
import { initials } from './format.js';

function PipelineFilter({ pipelineId, onPipeline, pipelines }) {
  ...
  const current = pipelineId ? pipelines.find((p) => p.id === pipelineId) : null;
  return (
    <div ref={ref} className="relative">
      <button type="button" onClick={() => setOpen((o) => !o)}
        ...
        <span className="w-5 h-5 rounded-md bg-surface2 text-text2 inline-flex items-center justify-center">
          <Layers size={11} />
        </span>
        <span className="font-semibold text-text truncate">{current ? current.name : 'Todos los CRMs'}</span>
        <span className="flex-1" />
        <ChevronDown size={11} className="text-text3" />
      </button>
      {open && (
        <div className="absolute right-0 top-full mt-1.5 min-w-[240px] bg-white border border-border rounded-lg shadow-xl p-1 z-50 max-h-[360px] overflow-y-auto">
          <button onClick={() => { onPipeline(null); setOpen(false); }}
                  ...
                  style={{ background: !pipelineId ? 'var(--color-blue-bg)' : 'transparent', color: !pipelineId ? 'var(--color-blue)' : 'var(--color-text)' }}>
            <span className="w-6 h-6 rounded-md bg-surface2 text-text2 inline-flex items-center justify-center">
              <Layers size={12} />
            </span>
            <span className="flex-1">Todos los CRMs</span>
            {!pipelineId && <Check size={13} />}
          </button>
          <div className="h-px bg-border my-1 mx-1" />
          {pipelines.length === 0 && (
            <div className="px-3 py-3 text-[11px] text-text3 text-center">Sin CRMs visibles.</div>
          )}
          {pipelines.map((p) => {
            const on = pipelineId === p.id;
            return (
              <button key={p.id}
                      onClick={() => { onPipeline(p.id); setOpen(false); }}
                      className="w-full flex items-center gap-2 px-2.5 py-2 rounded-md text-left text-[12.5px] font-semibold cursor-pointer hover:bg-surface2"
...
export default function DashboardFilters({
  vendor, setVendor, range, setRange,
  pipelineId, setPipelineId, pipelines = [],
  sellers, isAdmin, onEditTargets, generatedAt,
}) {
